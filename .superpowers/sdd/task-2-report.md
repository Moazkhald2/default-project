...
```tsx
export function PerfImage({ src, alt, width, height, priority=false }: { src:string; alt:string; width:number; height:number; priority?:boolean }) {
  return (
    <img
...
### `Heavy.tsx` verified:
```tsx
export default function Heavy(){ return <p className="mt-4 text-sm opacity-70">Lazy chunk — proves code-splitting. Check Network: this loads only when rendered.</p>; }
```
